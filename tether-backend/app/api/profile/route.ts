...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await adminClient
    .from("profiles")
    .select("id, username, full_name, bio, website, avatar_url, creator_stage, aspiration, platform_reason, metric_visibility, user_type, category")
    .eq("id", user.id)
    .maybeSingle();
...
export async function PUT(req: NextRequest) {
  ...
  const ALLOWED = ["username", "full_name", "bio", "website", "avatar_url", "creator_stage", "aspiration", "platform_reason", "metric_visibility", "user_type", "category"] as const;
