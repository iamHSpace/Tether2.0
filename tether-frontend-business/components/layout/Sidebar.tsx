...
export default function Sidebar({ email, name }: Props) {
    ...
    <aside className="w-60 shrink-0 h-screen sticky top-0 flex flex-col bg-white border-r border-gray-100">
      ...
      <div className="px-5 py-5 border-b border-gray-100">
        <a href="/" className="flex items-center gap-2.5">
          ...
          <div>
            <span className="font-bold text-gray-900 tracking-tight text-sm">Tether</span>
            <span className="ml-1 text-[10px] font-semibold text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded-full">Business</span>
          </div>
        </a>
      </div>
      ...
      <div className="px-3 pb-4 space-y-0.5 border-t border-gray-100 pt-3">
        ...
        <div className="mt-3 flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-gray-50">
          <div className="w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center shrink-0">
            <IconBriefcase size={13} className="text-brand-600" />
          </div>
          <div className="min-w-0">
            <p className="text-xs font-semibold text-gray-800 truncate">{name ?? "Business"}</p>
